		...
		async function handleCallback() {
			try {
				...
				if (session?.user) {
					...
					console.log(
						"No session found, waiting longer for session establishment...",
					);
					...
					while (retryCount < maxRetries) {
						...
						if (retrySession?.user) {
							console.log(
								`OAuth session established on retry ${retryCount + 1}:`,
						...
						retryCount++;
						console.log(
							`Retry ${retryCount}/${maxRetries} - still waiting for session...`,
						);
					}

					console.error(
						"No session received after OAuth callback with retries",
					);
					setStatus("error");
					setErrorMessage(
						"Session creation failed after multiple attempts. Please try signing in again.",
					);
					setTimeout(() => router.replace("/sign-in"), 3000);
				}
			} catch (error) {
