...
export default function TargetDetailScreen() {
  ...
  const targetSessions = sessions?.filter(session => session.target_id === id) || [];
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50">
        ...
        <View className="flex-row justify-between items-start">
          <View className="flex-1">
            ...
            <View className="flex-row items-center space-x-4">
              ...
              <Text className="text-gray-600">
                Type: {target.target_type.charAt(0).toUpperCase() + target.target_type.slice(1)}
              </Text>
            </View>
      ...
      {target.image_url && (
        <View className="bg-white p-4 border-b border-gray-200">
          <Text className="text-lg font-semibold text-gray-900 mb-2">Target Image</Text>
      ...
      <View className="p-4">
        <View className="flex-row justify-between items-center mb-4">
          ...
          <TargetButton
            variant="primary"
            size="sm"
            onPress={() => router.push(`/session/create?targetId=${target.id}` as any)}
          >
